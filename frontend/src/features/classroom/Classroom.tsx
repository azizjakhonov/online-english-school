...
const getActivityIcon = (type: ActivityType) => {
  ...
  switch (type) {
    ...
    case 'listening': return <Headphones size={11} />;
...
export default function Classroom() {
    ...
    socket.onmessage = (event) => {
      ...
      // VIDEO SYNC (unchanged)
        ...
        setAudioSyncState({
          ...
          action: data.type === 'AUDIO_STATE' ? 'AUDIO_STATE_UPDATE' : data.type // Map to existing component logic
        });
      }
    };
  ...
  const sendAudioAction = (type: string, data: any = {}) => {
    if (!socketRef.current) return;
    socketRef.current.send(JSON.stringify({
      type,
      payload: data,
  ...
  const handleExitRequest = () => {
    if (userRole === 'teacher') {
      // Teacher can submit wrap-up only when a lesson is active.
      if (!activeLesson?.id) {
  ...
  const renderActivityContent = () => {
    ...
    switch (type) {
      ...
      case 'video': {
        ...
        // SUPPORT DIRECT VIDEO FILES
        const vContent = safeContent as any;
        if (vContent.url || vContent.video_download_url) {
          const videoUrl = vContent.url ||
            (vContent.video_download_url?.startsWith('http') ? vContent.video_download_url : `${API_BASE_URL}${vContent.video_download_url}`);

          return (
            <div className="h-full w-full flex items-center justify-center bg-black p-4">
              <video
                controls={userRole === 'teacher'}
                className="w-full h-full max-w-4xl max-h-[80vh] rounded-xl"
                src={videoUrl}
              />
            </div>
          );
      ...
      case 'quiz':
        return (
          <Quiz
            key={activity.id}
            content={safeContent}
            isTeacher={userRole === 'teacher'} gameState={activeZoneData} onAction={(action: string, data: any) => sendZoneAction('quiz', action, data)}
          />
        );
      case 'pdf':
      ...
      case 'listening':
        return (
          <Listening
            ...
            onAction={(type: string, data: any) => sendAudioAction(type, data)}
          />
        );
      default:
        return (
