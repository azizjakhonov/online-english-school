        ...
        >
            ...
            <div className="min-w-0 flex-1">
                ...
                <div className="mt-2.5 h-1.5 rounded-full bg-gray-200 overflow-hidden" aria-hidden="true">
                    <div
                        className={`h-full rounded-full transition-all duration-500 ${isCorrect ? 'bg-emerald-400 w-full' : 'bg-red-400 w-0'
                            }`}
                    />
                </div>
            </div>
...
function Quiz({ content, isTeacher, gameState, onAction }: QuizProps) {
    ...
    useEffect(() => {
        ...
        if (gameState.selectedIndex !== undefined) {
            setSelectedIndex(gameState.selectedIndex);
        }
        if (gameState.submitted !== undefined) {
            setSubmitted(gameState.submitted);
        }
    }, [gameState]);
    ...
    }, [submitted, isTeacher, onAction]);

    const handleSubmit = useCallback(() => {
        setSubmitted(true);
        onAction?.('SUBMIT_QUIZ', { submitted: true, selectedIndex });
                ...
                <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        {isTeacher ? (
                            ...
                            <span
                                className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full ${isAnswerCorrect
