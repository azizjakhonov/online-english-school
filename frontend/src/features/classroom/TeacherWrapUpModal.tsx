...
export default function TeacherWrapUpModal({
    ...
    const normalizedLessonId =
        typeof lessonId === 'number'
            ...
            : lessonId
                ? Number(lessonId)
                : NaN;
    const hasValidLessonId = Number.isFinite(normalizedLessonId) && normalizedLessonId > 0;
    ...
    const canSave =
        hasValidLessonId &&
        selectedStatus !== null &&
        (!notesRequired || teacherNotes.trim().length > 0);
    ...
    const handleSave = async () => {
        if (!canSave) return;
        if (!hasValidLessonId) {
            setError('No active lesson found for wrap-up. Please return to dashboard.');
            return;
        }
        setSaving(true);
        setError(null);
        try {
            ...
            await api.patch(`/api/lessons/${normalizedLessonId}/wrap-up/`, {
            ...
            if (showHomework && selectedHomeworkId && homeworkDue) {
                try {
                    await api.post(`/api/homework/assign/${normalizedLessonId}/`, {
